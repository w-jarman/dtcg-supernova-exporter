import { FileHelper, GeneralHelper, ThemeHelper, FileNameHelper, StringCase } from "@supernovaio/export-utils"
...
import { StyleDictionaryValue, tokenToStyleDictionaryValue } from "../helpers/StyleDictionaryValueHelper"

/**
 * Creates a value object for a token, either as a simple value or themed values
 */
function createTokenValue(
  value: StyleDictionaryValue,
  token: Token,
  theme?: TokenTheme
): any {
...
function processTokensToObject(
  ...
  sortedTokens.forEach(token => {
    ...
    // Preserve the semantic token value for Style Dictionary to transform per platform.
    const value = tokenToStyleDictionaryValue(token, mappedTokens, {
